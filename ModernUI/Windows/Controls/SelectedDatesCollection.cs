...
namespace ModernUI.Windows.Controls
{
    ...
    public sealed class SelectedDatesCollection : ObservableCollection<DateTime>
    {
        ...
        protected override void ClearItems()
        {
            if (!IsValidThread())
            {
                throw new NotSupportedException("这种类型的集合不支持它的sourcecollection的变化从一个不同的线程的线程调度。");
            }
        ...
        protected override void InsertItem(int index, DateTime item)
        {
            ...
            if (!this.Contains(item))
            {
                ...
                {
                    throw new ArgumentOutOfRangeException("SelectedDate的值无效。");
                }
            }
        }
        ...
        private bool CheckSelectionMode()
        {
            ...
            {
                throw new InvalidOperationException("当选择模式为空时，不能设置SelectedDate 属性");
            }

            if (this._owner.SelectionMode == CalendarSelectionMode.SingleDate && this.Count > 0)
            {
                throw new InvalidOperationException("SelectedDates集合只能在多选模式的改变。在单选择模式使用SelectedDate。");
            }
